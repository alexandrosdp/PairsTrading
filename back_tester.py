    ...
    for t, current_index in enumerate(z_index):
        ...
        if position == 0:
            ...
            if abs(current_z) >= stop_loss_threshold: # We can take absolute to account for either side
                ...
                positions.append(0)
                #print("⚠️ Stop-out triggered during lockout")
                continue
...
def compute_sortino_ratio(initial_capital, trade_profits, risk_free_rate=0.0):
    ...
    The Sortino ratio is a measure of risk-adjusted
    return that penalizes only downside volatility.
    Parameters:
    initial_capital (float): The initial capital of the portfolio.
    trade_profits (list): A list of trade profits.
    risk_free_rate (float): The risk-free rate of return.
    Returns:
    sortino_ratio (float): The Sortino ratio of the strategy.
    """
    trade_profits = np.array(trade_profits)
...
def simulate_strategy_trade_pnl(trade_entries, trade_exits, initial_capital, beta_series, tx_cost):
    ...
    for entry, exit in zip(trade_entries, trade_exits):
        ...
        # Append transaction costs for diagnostics.
        transaction_costs.append(total_fees)
    ...
    return trade_profits,transaction_costs, net_trade_profits_S1, net_trade_profits_S2,cumulative_profit_series, entry_times, exit_times
